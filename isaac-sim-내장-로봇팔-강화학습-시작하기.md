...
`isaaclab.sh` 파일 자체가 GUI를 결정하는 것은 아니다. 뒤에 붙이는 `train`, `play` 명령과 visualizer 옵션이 실행 방식을 결정한다.

현재 Isaac Lab 3.x에서는 다음처럼 사용한다.

```bash
# Omniverse Isaac Sim GUI/Kit 창을 표시
...
./isaaclab.sh train --rl_library rsl_rl \
  --task Isaac-Reach-Franka-v0 \
  --num_envs 1024 --max_iterations 1000 \
  --viz none
```

`--headless`도 일부 명령에서 동작하지만 현재 branch에서는 deprecated 옵션이다. 문서에서는 `--viz kit`와 `--viz none`을 우선 사용한다. `--viz kit`을 생략하면 기본적으로 visualizer가 선택되지 않아 headless로 실행될 수 있다.
...
# 짧은 Reach 확인
./isaaclab.sh train --rl_library rsl_rl \
  --task Isaac-Reach-Franka-v0 --num_envs 64 --max_iterations 10 --viz kit

# ROBOTIS custom Reach 예시
./isaaclab.sh train --rl_library rsl_rl \
  --task Isaac-Reach-OpenManipulatorX-v0 --num_envs 64 --max_iterations 10 --viz kit

# ROBOTIS custom Lift-Cube 예시
./isaaclab.sh train --rl_library rsl_rl \
  --task Isaac-Lift-Cube-OpenManipulatorX-v0 --num_envs 1024 \
  --max_iterations 2000 --viz none
```
